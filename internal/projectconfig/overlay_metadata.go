...
const (
	...
	// environment: toolchain and mock adjustments, workarounds for packages not yet
	// imported into AZL, and similar compatibility fixes that are not themselves
	// backports.
...
type OverlayMetadata struct {
	...
	Category OverlayCategory `toml:"category" json:"category" jsonschema:"required,enum=backport-dist-git,enum=azl-pruning,enum=azl-compatibility,enum=azl-branding-policy,enum=azl-disable-flaky-tests,enum=azl-disable-unsupported-tests,enum=azl-security-compliance,enum=azl-release-management,enum=azl-platform-adaptation,title=Category,description=Classification of the overlay's intent"`
	...
	BugLinks []string `toml:"bugs,omitempty" json:"bugs,omitempty" validate:"omitempty,dive,http_url" jsonschema:"title=Issue-tracker links,description=URLs of related issue-tracker entries"`
...
func (m *OverlayMetadata) Validate() error {
	...
	// Field-level constraints (for example 'bug' entries must be http(s) URLs) are
