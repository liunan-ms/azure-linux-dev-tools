...
type ComponentConfig struct {
	...
	// filename (lexicographic) order. Duplicate matches are de-duplicated, preserving
	// first occurrence. The resulting overlays are appended to [ComponentConfig.Overlays]
	...
	OverlayFiles []string `toml:"overlay-files,omitempty" json:"overlayFiles,omitempty" table:"-" jsonschema:"title=Overlay files,description=Glob patterns (relative to the component config file) matched against the filesystem to locate per-file overlay documents at load time" fingerprint:"-"`
